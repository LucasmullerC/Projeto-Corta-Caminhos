...
public class Dijkstra {
	...
	public List<Vertice> encontrarMenorCaminhoDijkstra(Grafo grafo, Vertice v1, Vertice v2) {
		...
		while (!this.naoVisitados.isEmpty()) {
			...
			/*
			 * Para cada vizinho (cada aresta), calcula-se a sua possivel distancia, somando
			 * a distancia do vertice atual com a da aresta correspondente. Se essa
			 * distancia for menor que a distancia do vizinho, esta eh atualizada.
			 */
			for (int i = 0; i < atual.getArestas().size(); i++) {
				...
				if (!vizinho.verificarVisita()) {
					...
					if (vizinho.getDistancia() > (atual.getDistancia() + atual.getArestas().get(i).getPeso())) {
						...
						/*
						 * Se o vizinho eh o vertice procurado, e foi feita uma mudanca na distancia, a
						 * lista com o menor caminho anterior eh apagada, pois existe um caminho menor
